...
// import { useRouter } from 'next/navigation';
// import { useTheme } from 'next-themes';
// import { Star, Heart, Clock, Users, BookOpen, PlayCircle, Search, Grid, List, X } from 'lucide-react';
// import Image from 'next/image';
// import axios from 'axios';
// import { motion } from 'framer-motion';
...
//   isDark: boolean;
// }> = ({ isOpen, onClose, recommendations, isDark }) => {
...
//     >
//       <div className={`rounded-xl p-6 max-w-2xl w-full ${isDark ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
...
//                 <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{rec.description}</p>
//                 <p className="text-sm">Category: {rec.category} | Level: {rec.level}</p>
//                 <p className="text-sm">Tags: {rec.tags.join(', ')}</p>
//               </div>
//             ))}
//           </div>
//         )}
...
//   const fetchRecommendations = useCallback(async (course: Course) => {
//     try {
//       const response = await axios.post<ApiResponse<Recommendation[]>>(`${baseUrl}/api/v1/recommend-courses`, {
//         name: course.name,
//         tags: course.tags,
//       });
//       if (response.data.success) {
