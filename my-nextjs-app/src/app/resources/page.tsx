...
const Page = () => {
  ...
  const resources: Resource[] = [
    {
      title: "Tutorials",
      description: "Step-by-step guides on web development, AI, and software engineering, updated as of July 18, 2025.",
      link: "/resources/tutorials",
      icon: "📚",
    },
    {
      title: "Documentation",
      description: "Comprehensive docs for MERN stack, Python, and system programming, last revised 10:22 AM PKT today.",
      link: "/resources/documentation",
      icon: "📖",
    },
    {
      title: "Tools & IDEs",
      description: "Access to VS Code, PyCharm, and networking simulators with setup instructions.",
      link: "/resources/tools",
      icon: "🛠️",
    },
    {
      title: "Community Support",
      description: "Join forums and live Q&A sessions with experts, available 9 AM - 6 PM PKT.",
      link: "/resources/community",
      icon: "💬",
    },
    {
      title: "Project Templates",
      description: "Ready-to-use templates for e-commerce, LMS, and AI projects, released July 2025.",
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? `bg-gradient-to-b from-[#121e4a] to-black text-white` : `bg-gradient-to-b from-[#121e4a]/20 to-white text-gray-900`} flex flex-col items-center p-4 sm:p-6`}>
      ...
      <div className="w-full max-w-4xl text-center mb-6 sm:mb-12">
        <h1 className={`text-3xl sm:text-5xl font-bold mb-4 ${isDarkMode ? "bg-gradient-to-r from-purple-300 to-indigo-400 bg-clip-text text-transparent" : "bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent"} tracking-wide`}>
          Resources Hub
        </h1>
        <p className={`text-sm sm:text-lg ${isDarkMode ? "text-gray-300" : "text-gray-600"} max-w-xs sm:max-w-2xl mx-auto leading-relaxed`}>
          Your gateway to cutting-edge tutorials, documentation, tools, community support, project templates, and expert staff profiles—updated daily as of July 18, 2025.
        </p>
      </div>
      <div className="w-full max-w-4xl mb-6 sm:mb-12">
        <h2 className={`text-2xl sm:text-3xl font-semibold mb-4 ${isDarkMode ? "text-white" : "text-gray-900"}`}>Available Resources</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {resources.map((resource) => (
            <Link
              key={resource.link}
              href={resource.link}
              className={`p-4 sm:p-6 rounded-xl shadow-lg transform transition-all duration-300 hover:scale-105 ${isDarkMode ? "bg-gray-800 border-gray-700 hover:bg-gray-700" : "bg-white border-gray-200 hover:bg-gray-50"} border text-center`}
            >
              <span className={`text-2xl sm:text-3xl mb-2 sm:mb-4 block ${isDarkMode ? "text-purple-300" : "text-indigo-600"}`}>{resource.icon}</span>
              <h3 className={`text-lg sm:text-xl font-semibold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}>{resource.title}</h3>
              <p className={`mt-1 sm:mt-2 ${isDarkMode ? "text-gray-400" : "text-gray-600"} text-xs sm:text-sm`}>{resource.description}</p>
            </Link>
          ))}
        </div>
      </div>
      <div className="w-full max-w-4xl">
        <h2 className={`text-2xl sm:text-3xl font-semibold mb-4 ${isDarkMode ? "text-white" : "text-gray-900"}`}>Our Expert Team</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {staff.map((member) => (
            <div
              key={member.name}
