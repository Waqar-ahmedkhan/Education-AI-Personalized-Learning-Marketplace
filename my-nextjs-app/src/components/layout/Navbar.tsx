...
import { Sun, Moon, Home, Info, BookOpen, Folder, LayoutDashboard } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <Link href="/" className="flex items-center gap-2" aria-label="EduAI Home">
          <Image
            src="/logo.png"
            alt="EduAI Logo"
            width={50}
      ...
      >
        ...
        {userRole && (
          <Link
            ...
            className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg text-xs font-medium transition-transform duration-200 hover:scale-105 ${
              pathname === (userRole === 'admin' ? '/admin-dashboard' : '/user-dashboard')
                ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-100 dark:bg-indigo-900/20'
                : isDarkMode
                ? 'text-gray-400 hover:text-white hover:bg-gray-800'
                : 'text-gray-600 hover:text-indigo-600 hover:bg-gray-100'
